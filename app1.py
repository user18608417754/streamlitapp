...
🔸 Welcome to the Shiny App for Neurosyphilis Diagnosis.<br>
🔸 You can use this to predict the risk of developing a neurosyphilis outcome in syphilis patients without HIV infection.<br>
🔸 This page exhibits Model 2. Model 2 uses consensus definition (agreement by at least three of five guidelines) as predictive outcome.</div>"""
...
model_1 = joblib.load("new_model/model1.pkl")
model_2 = joblib.load("new_model/model2.pkl")
...
with tabs[0]:
    ...
    #col[1].markdown('''<div style="color: black; border-radius: 8px; padding: 8px; border: 1px solid black;">This page exhibits Model 2. Model 2 uses consensus definition (agreement by at least three of five guidelines) as predictive outcome.</div>''', unsafe_allow_html=True)
    ...
    pre1 = model_1.predict_proba(pre1)
...
with tabs[1]:
    ...
    pre2 = model_2.predict_proba(pre2)
